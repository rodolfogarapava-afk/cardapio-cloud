    ...
    <main className={`app-shell${publicMenu ? " public-menu-shell" : ""}`}>
      ...
      <div className={`workspace${publicMenu ? " public-menu-workspace" : ""}`}>
        ...
        <section className={`content${!publicMenu && systemView === null && (!tenantNavigation || tenantNavigation.page === "operation") ? ` client-menu-content${compactMenu ? " compact-menu" : ""}` : ""}`}>
          ...
          systemView === "reports" ? <IntegratedReports storeName={effectiveTenantName||"Cardápio Digital"} sales={salesHistory} expenses={expenses} commands={savedCommands} products={products} sync={operationsSync} /> : <>
...
function IntegratedReports({storeName,sales,expenses,commands,products,sync}:{storeName:string;sales:IntegratedSale[];expenses:IntegratedExpense[];commands:IntegratedCommand[];products:Product[];sync:OperationsSyncView}) {
  ...
  const formatPeriodDate=(value:string)=>{const [year,month,day]=value.split("-");return year&&month&&day?`${day}/${month}/${year}`:value};
  const periodLabel=period==="today"?"Hoje":period==="7d"?"Últimos 7 dias":period==="month"?"Mês atual":`${formatPeriodDate(from)} a ${formatPeriodDate(to)}`;
  const downloadXls=()=>{
    const esc=(value:unknown)=>String(value??"").replace(/[&<>]/g,char=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[char]||char));
    const rows=[`<tr><th colspan="4">${esc(storeName)} · Relatório financeiro${includePeriod?` · ${esc(periodLabel)}`:""}</th></tr>`,`<tr><td>Entradas</td><td>${revenue}</td><td>Saídas</td><td>${costs}</td></tr>`];
    if(includeCharts){rows.push("<tr><th>Forma de pagamento</th><th>Valor</th><th>Participação</th><th></th></tr>",...methodTotals.map(row=>`<tr><td>${esc(row.method)}</td><td>${row.total}</td><td>${((row.total/pieTotal)*100).toFixed(1)}%</td><td></td></tr>`));}
  ...
  return <div className="integrated-view modern-reports">
    ...
    <div className="report-controls">
      <div className="report-mode-tabs" aria-label="Tipo de relatório"><button type="button" className={mode==="simple"?"active":""} aria-pressed={mode==="simple"} onClick={()=>setMode("simple")}>SIMPLIFICADA</button><button type="button" className={mode==="advanced"?"active":""} aria-pressed={mode==="advanced"} onClick={()=>setMode("advanced")}>AVANÇADA</button></div>
      <div className="period-tags" aria-label="Período do relatório">{([['today','Hoje'],['7d','Semana'],['month','Mês'],['custom','Período']] as [Period,string][]).map(([key,label])=><button type="button" className={period===key?"active":""} aria-pressed={period===key} onClick={()=>setPeriod(key)} key={key}>{label}</button>)}</div>
      {period==="custom"&&<div className="period-custom"><label><span>De</span><input aria-label="Data inicial" type="date" value={from} max={to} onChange={event=>{const value=event.target.value;setFrom(value);if(value>to)setTo(value)}}/></label><label><span>Até</span><input aria-label="Data final" type="date" value={to} min={from} onChange={event=>{const value=event.target.value;setTo(value);if(value<from)setFrom(value)}}/></label></div>}
    </div>
